...
export default function Home_screen({ navigation }) {
  ...
  const renderItem = (type) => {
    ...
    var listRender = listProduct.filter(item => item.type == type)
    listRender = listRender.filter((item, i) => i <= 3)
    ...
    //     renderItem={({ item }) => {
    //       const { price, name, plantType, image } = item;
    //       return <ItemProduct_Components price={price} name={name} plantType={plantType} key={item._id} img={image[0]} item={item} navigation={navigation} />
    //     }}
    //     numColumns={2}
