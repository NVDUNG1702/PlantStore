...
export default function Signup_screen({ navigation }) {
  ...
  const signup = async () => {
    // await storage.removeData('account')
    const validation = !errPass && !errMail && !errName && !errPhoneNumber;
    if (!validation) {
      return;
    }

    if (pass == "" || mail == "" || name == "" || phoneNumber == "") {
      return;
    }
    // setLoading(true);


    let accounts = await storage.getData("account");
    ...
    if (accounts == null || accounts.length == 0) {
      ...
      accounts = [user]
      await storage.storeData('account', accounts)
    } else {
      const userFind = accounts.find((user, i) => {
        return (user.mail == mail || user.phoneNumber == phoneNumber)
        console.log(i);
      ...
      if (userFind) {
        Alert.alert("Error", "Email or PhoneNumber đã tồn tại!!",
          [
            {
              style: 'cancel',
              text: 'ok'
