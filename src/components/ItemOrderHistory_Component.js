...
export default function ItemOrderHistory_Component({ item }) {
    ...
    var listCart = useSelector((state) => selectCartByUid(state, userLogin.uid)).filter(i => i.id_order == item.id)
                ...
                <View style={{ width: '90%', height: '200' }}>
                    <FlatList
                        ...
                        renderItem={({ item }) => {
                            ...
                            return (
                                <View style={[{
                                    width: '100%', flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', marginVertical: 10,
